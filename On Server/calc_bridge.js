...
// --- NEW: Validation helpers from Fusion Calc ---
...
function App() {
  ...
  // --- NEW: State for inline validation feedback ---
  ...
  // --- UPDATED: Email handler with inline validation ---
  const handleEmail = async () => {
    setIsSending(true);
    setSendStatus(null);
    setValidationError(""); // Clear previous errors on a new attempt

    // 1. Check for valid calculation first
    if (!calculationResults || !isValid) {
      setValidationError("Please ensure all loan details are valid before sending.");
      setIsSending(false);
      return;
    }

    // 2. Check all client fields are filled
    if (!clientName.trim() || !contactNumber.trim() || !emailAddress.trim()) {
      setValidationError("Please complete all client fields before sending.");
      setIsSending(false);
      return;
    }
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <InputField
              ...
              value={contactNumber}
              onChange={(e) => setContactNumber(cleanDigits(e.target.value))}
              type="tel"
            ...
            />
            <div className="flex flex-col">
                <label className="text-sm text-gray-700 font-semibold mb-1 hidden lg:block">&nbsp;</label>
                <button
                  onClick={handleEmail}
                  disabled={isSending}
                  className={`flex w-full items-center justify-center space-x-2 px-6 py-3 font-semibold rounded-lg shadow-md transition-transform transform hover:scale-105 text-white ${
                    isSending
                      ? "bg-gray-500 cursor-not-allowed"
                      : "bg-[#ED8B00] hover:bg-[#FBA429] focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                  }`}
                >
                  {isSending && (
                    <svg
          ...
          {validationError && (
            <div className="mt-4 p-3 text-center text-sm font-semibold text-red-800 bg-red-100 border border-red-300 rounded-lg">
                {validationError}
            </div>
          )}
          ...
          {displayResults && (
            <div className="border border-gray-200 rounded-xl shadow-sm overflow-hidden bg-white">
              ...
              <div className="grid grid-cols-[1fr,1fr,1fr] gap-2">
                ...
                <PillHeader className="bg-[#ED8B00]">Fixed Bridge</PillHeader>
                <PillHeader className="bg-[#002855]">Variable Bridge</PillHeader>
              </div>
...
const rootElement = document.getElementById("calc-root");
if (rootElement) {
